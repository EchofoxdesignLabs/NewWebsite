...
export default function WorkScene({ sectionEl }) {
  ...
  useEffect(() => {
    ...
    scene.background = bg;
    scene.fog = new THREE.Fog(bg, 10, 80);
  ...
  useEffect(() => {
    ...
    const SCALE = 0.8; // 0.7–0.85 is a sweet spot
  ...
  /** Render */
  useFrame((_, delta) => {
    // Parallax (only when not animating)
    if (!isAnimatingRef.current) {
      const pf = 0.5; // position factor (subtle drift)
      const tx = baseCamRef.current.x + mouseRef.current.x * pf;
      const ty = baseCamRef.current.y + mouseRef.current.y * pf;

      camera.position.x += (tx - camera.position.x) * 0.05;
      camera.position.y += (ty - camera.position.y) * 0.05;

      const pa = 0.011;
      const ry = mouseRef.current.x * pa;
      const rx = mouseRef.current.y * pa;
      parallaxGroupRef.current.rotation.y += (ry - parallaxGroupRef.current.rotation.y) * 0.001;
      parallaxGroupRef.current.rotation.x += (rx - parallaxGroupRef.current.rotation.x) * 0.001;
    }
